...
const SidebarNewUsers = ({ open, toggleSidebar }) => {
  ...
  // console.log("valalalalaidddiidiid")
      ...
      dispatch(
        addUser({
          name: data.full_name,
          thumbnail: data.full_name,
          time: data.full_name,
          vehicle: data.full_name,
          departurePlace: data.full_name,
          destination: data.full_name,
          schedule: data.full_name,
          policy: data.full_name
        })
      )
  //   } else {
      ...
      <Form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className='mb-1'>
          ...
          <Controller
            ...
            control={control}
            render={({ field }) => (
              <Input id='email' placeholder='Time' invalid={errors.email && true} {...field} />
            )}
          />
        </div>
        <div className='mb-1'>
          ...
          <Controller
            ...
            control={control}
            render={({ field }) => (
              <Input id='address' placeholder='Vehicle' invalid={errors.address && true} {...field} />
            )}
          />
        </div>
        <div className='mb-1'>
          <Label className='form-label' for='phone'>
            Departure Place<span className='text-danger'>*</span>
          </Label>
          <Controller
            name='phone'
            control={control}
            render={({ field }) => (
              <Input id='phone' placeholder='Departure Place' invalid={errors.phone && true} {...field} />
            )}
          />
        </div>
        <div className='mb-1'>
          <Label className='form-label' for='username'>
            Destinision <span className='text-danger'>*</span>
          </Label>
          <Controller
            name='username'
            control={control}
            render={({ field }) => (
              <Input id='username' placeholder='Destinision' invalid={errors.username && true} {...field} />
            )}
          />
        </div>
        {/* <div className='mb-1'>
